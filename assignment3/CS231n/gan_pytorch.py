...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss_F=bce_loss(logits_fake, torch.zeros(N).type(dtype))
    loss_R=bce_loss(logits_real, torch.ones(N).type(dtype))
    loss = loss_R+loss_F
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_real= torch.mean(torch.square(scores_real-1))/2
    loss_fake = torch.mean(torch.square(scores_fake))/2
